...
זמני ביצוע: Starter (דף נחיתה) עד 5 ימי עסקים ולעיתים פחות (תלוי במורכבות), Business 1-2 שבועות, Premium 2-4 שבועות.
דומיין ואחסון אינם כלולים במחירי החבילות. דומיין: רכישה וניהול — 150 ₪/שנה כתוספת, נרשם על שם הלקוח. אחסון: משולם ישירות לספק האחסון לפי המסלול (לאתר עסקי קטן לרוב עשרות שקלים בחודש) — אנחנו עוזרים לבחור ולהקים. הקוד והדומיין בבעלות הלקוח.
טכנולוגיות לפי צורך: פיתוח מותאם אישית (Next.js, React, TypeScript, Node.js); אתרי ניהול תוכן עצמאי (WordPress, Elementor, Wix Studio); חנויות (Shopify, WooCommerce); אוטומציות ו-AI (צ'אטבוטים, WhatsApp API, CRM, Make).
